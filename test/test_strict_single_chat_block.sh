...
if [[ "\$url" == *"/json/list"* ]]; then
  printf '%s\n' '[{"id":"tab1","url":"$target_url","title":"Target","webSocketDebuggerUrl":"ws://fake"},{"id":"tab2","url":"$other_url","title":"Other","webSocketDebuggerUrl":"ws://fake2"}]'
  exit 0
fi
...
if args.precheck_only:
    ...
    raise SystemExit(10)
print("assistant should not happen", flush=True)
EOF
...
set +e
out="$("$SCRIPT" --chatgpt-url "$target_url" --prompt "strict single check" 2>&1)"
st=$?
set -e
[[ "$st" -eq 78 ]]
echo "$out" | rg -q -- 'E_MULTIPLE_CHAT_TABS_BLOCKED'
