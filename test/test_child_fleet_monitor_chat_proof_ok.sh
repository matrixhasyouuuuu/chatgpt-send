...
printf '%s\n' "CHILD_BROWSER_USED: yes ; EVIDENCE: https://chatgpt.com/c/aaa" >"$run/r1.log"
printf '%s\n' "CHILD_RESULT: done" >"$run/r1.last.txt"
printf '%s\n' "0" >"$run/r1.exit"
cat >"$run/child_result.json" <<'JSON'
{"status":"OK","exit_code":0,"pinned_route_url":"https://chatgpt.com/c/aaa?x=1","specialist_chat_url":"https://chatgpt.com/c/aaa/"}
JSON
...
if obj.get("chat_ok_total") != 1:
    raise SystemExit(f"expected chat_ok_total=1, got {obj.get('chat_ok_total')}")
if obj.get("chat_mismatch_total") != 0:
    raise SystemExit(f"expected chat_mismatch_total=0, got {obj.get('chat_mismatch_total')}")
...
row = agents[0]
if row.get("chat_proof") != "ok":
    raise SystemExit(f"expected chat_proof=ok, got {row.get('chat_proof')}")
...
if row.get("observed_chat_url_norm") != "https://chatgpt.com/c/aaa":
