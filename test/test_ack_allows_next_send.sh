...
out1="$("$SCRIPT" --chatgpt-url "https://chatgpt.com/c/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa" --prompt "first message" 2>&1)"
echo "$out1" | rg -q -- 'assistant reply for first message'

set +e
out_block="$("$SCRIPT" --chatgpt-url "https://chatgpt.com/c/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa" --prompt "second message" 2>&1)"
st_block=$?
set -e
[[ "$st_block" -eq 12 ]]
echo "$out_block" | rg -q -- 'E_REPLY_UNACKED_BLOCK_SEND'

out_ack="$("$SCRIPT" --chatgpt-url "https://chatgpt.com/c/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa" --ack 2>&1)"
echo "$out_ack" | rg -q -- 'ACK_WRITE'
echo "$out_ack"

out2="$("$SCRIPT" --chatgpt-url "https://chatgpt.com/c/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa" --prompt "second message" 2>&1)"
echo "$out2" | rg -q -- 'assistant reply for second message'

[[ "$(cat "$FAKE_SEND_COUNT")" == "2" ]]
