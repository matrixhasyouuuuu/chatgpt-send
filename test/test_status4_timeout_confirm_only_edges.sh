...
case = os.environ.get("FAKE_CASE", "prompt_absent_after_timeout")
...
def emit_fetch(prompt: str, include_prompt: bool, assistant_after: bool, assistant_text: str = ""):
    ...
    payload = {
        ...
        "checkpoint_id": "SPC-2099-01-01T00:00:00Z-" + (h(user_text or 'none')[:8] if (user_text or assistant_text) else "none"),
...
if args.fetch_last:
    ...
    # call1: recovery; call2: final pre-send dedupe; call3: status4 confirm-only
    if n <= 2:
        emit_fetch(prompt, include_prompt=False, assistant_after=False, assistant_text="")
        raise SystemExit(0)
    if case == "prompt_absent_after_timeout":
        emit_fetch(prompt, include_prompt=False, assistant_after=False, assistant_text="")
        raise SystemExit(0)
    if case == "transient_zero_messages_then_prompt_present":
        if n == 3:
            emit_fetch(prompt, include_prompt=False, assistant_after=False, assistant_text="")
        else:
            emit_fetch(prompt, include_prompt=True, assistant_after=True, assistant_text="reply after confirm loop")
        raise SystemExit(0)
    if case == "confirm_fetch_fail_after_timeout":
        print("FETCH_LAST forced fail after timeout", flush=True)
        raise SystemExit(4)
    emit_fetch(prompt, include_prompt=False, assistant_after=False, assistant_text="")
    raise SystemExit(0)
...
run_case() {
  ...
  local out_var="$3"
  local st_var="$4"
  rm -f "$state_dir/"*.txt 2>/dev/null || true
  rm -f "$root/state/ack.json" 2>/dev/null || true
  rm -f "$root/state/timeout_budget_events.log" 2>/dev/null || true
  export FAKE_CASE="$case_name"
  export FAKE_STATE_DIR="$state_dir"
  set +e
  local out
  out="$("$SCRIPT" --chatgpt-url "https://chatgpt.com/c/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa" --prompt "$prompt" 2>&1)"
  local st=$?
  set -e
...
# Scenario 1: confirm-only path cannot confirm prompt presence -> fail-safe 81
out1=""
st1=""
run_case "prompt_absent_after_timeout" "status4 confirm-only prompt absent" out1 st1
[[ "$st1" -eq 81 ]]
echo "$out1" | rg -q -- 'RUNTIME_EVAL_TIMEOUT'
echo "$out1" | rg -q -- 'E_CDP_TIMEOUT_RETRY attempt=1 decision=confirm_only no_resend=1'
echo "$out1" | rg -q -- 'SEND_RETRY_VETO_INTRA_RUN reason=intra_run_retry_after_dispatch prompt_present=0'
echo "$out1" | rg -q -- 'E_SEND_RETRY_VETO_INTRA_RUN reason=prompt_not_confirmed_no_resend'
echo "$out1" | rg -q -- 'CONFIRM_LOOP attempt=1/2 result=unstable'
assert_no_second_send "$out1"

# Scenario 2: transient zero-message confirm fetch becomes prompt_present on the
# next read-only confirm attempt -> reuse path, still no resend.
out2=""
st2=""
run_case "transient_zero_messages_then_prompt_present" "status4 transient zero messages" out2 st2
[[ "$st2" -eq 0 ]]
echo "$out2" | rg -q -- 'RUNTIME_EVAL_TIMEOUT'
echo "$out2" | rg -q -- 'CONFIRM_LOOP attempt=1/2 result=unstable'
echo "$out2" | rg -q -- 'CONFIRM_LOOP attempt=1/2 result=prompt_present'
echo "$out2" | rg -q -- 'SEND_RETRY_VETO_INTRA_RUN reason=intra_run_retry_after_dispatch prompt_present=1'
echo "$out2" | rg -q -- 'REUSE_EXISTING reason=intra_run_retry_after_dispatch'
assert_no_second_send "$out2"

# Scenario 3: confirm-only fetch_last itself fails -> fail-closed 79
out3=""
st3=""
run_case "confirm_fetch_fail_after_timeout" "status4 confirm fetch fail" out3 st3
[[ "$st3" -eq 79 ]]
echo "$out3" | rg -q -- 'RUNTIME_EVAL_TIMEOUT'
echo "$out3" | rg -q -- 'E_CDP_TIMEOUT_RETRY attempt=1 decision=confirm_only no_resend=1'
echo "$out3" | rg -q -- 'E_SEND_RETRY_VETO_INTRA_RUN reason=confirm_fetch_last_failed'
assert_no_second_send "$out3"
